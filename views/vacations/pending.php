<div class="row content">
	<div class="col-sm-4">
		<h2 id="title"><small>Pending Vacations</small></h2>
	</div>
...
if($vacations->data){
	echo "<table class='table table-striped'><tr><th>Name</th><th>Requested on</th><th>From</th><th>Until</th><th>Total Requested Days</th><th>Remaining days</th><th>actions</th></tr>";
	...
	if($pages>1){
		...
		if($page>1){
			echo "<li><a href='#' onclick=\"load_page('vacations','pending',$page-1,'$f');\">«</a></li>";
		}
		for($p=1; $p<=$pages; $p++){
			if($page and $page==$p){
				$class='active';
			...
			echo "<li class='$class'><a href='#' data-id='$p' onclick=\"load_page('vacations','pending',$p,'$f');\">$p</a></li>";
		}
		if($page!=$pages){
			echo "<li><a href='#' onclick=\"load_page('vacations','pending',$page+1,'$f');\">»</a></li>";
		}
	...
	echo "<div class='alert alert-danger'>There are no Pending Vacations!</div>";
}